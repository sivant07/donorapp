...
def check(email):
    url = "https://gr22uncx9e.execute-api.us-east-2.amazonaws.com/plasma/getdata?email="+email
...
def register():
    ...
    if('errorType' in check(x[1])):
        url = "https://gr22uncx9e.execute-api.us-east-2.amazonaws.com/plasma/registration?"+params
...
@app.route('/stats')
def stats():
    url = "https://gr22uncx9e.execute-api.us-east-2.amazonaws.com/plasma/getbloodgroupdata"
    response = requests.get(url)
    r = response.json()
    print(r)
...
def requested():
    ...
    #print(bloodgrp)
    url = "https://gr22uncx9e.execute-api.us-east-2.amazonaws.com/plasma/requestonbloodgroup?blood="+bloodgrp
    status = requests.request("GET",url)
    a=status.json()
    print(a)
    phone=[]
    for i in a:
        url ="https://www.fast2sms.com/dev/bulk?authorization=a0rMJykGSXEqxwFhNcYoB39p4APuvbKgdfRHVtis71ICzWn5D8oMkvfGcV7TXQeCEBO0KwZyuLUFPxbN&sender_id=FSTSMS&message=Need Plasma of your blood group&language=english&route=p&numbers="+str(i['phone'])
